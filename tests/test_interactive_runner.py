...
from test_ai.cli.interactive_runner import (
    InteractiveRunner,
    WorkflowInput,
    WorkflowTemplate,
)
...
class TestWorkflowTemplate:
    ...
    def test_defaults(self):
        wt = WorkflowTemplate(
            id="test", name="Test", description="desc", category="Dev"
        )
        assert wt.inputs == []
        assert wt.tags == []
...
class TestInteractiveRunner:
    ...
    def runner(self):
        ...
        with patch("test_ai.cli.interactive_runner.get_output") as mock_get:
    ...
    def test_check_dependencies_no_inquirer(self):
        with patch("test_ai.cli.interactive_runner.INQUIRER_AVAILABLE", False):
            with patch("test_ai.cli.interactive_runner.get_output") as mock_get:
                mock_output = MagicMock()
                mock_get.return_value = mock_output
                InteractiveRunner()
    ...
    def test_gather_inputs_all_provided(self, runner):
        template = WorkflowTemplate(
            ...
            category="C",
            inputs=[
                WorkflowInput(name="x", description="X", required=False, default="val")
            ],
        )
        with patch("test_ai.cli.interactive_runner.INQUIRER_AVAILABLE", False):
    ...
    def test_execute_workflow(self, mock_sleep, runner):
        template = WorkflowTemplate(
            id="feat", name="Feature", description="d", category="C"
        )
...
class TestModuleFunctions:
    ...
    @patch("test_ai.cli.interactive_runner.InteractiveRunner")
    def test_run_interactive(self, mock_cls):
        from test_ai.cli.interactive_runner import run_interactive
    ...
    def test_list_workflows(self, mock_cls):
        from test_ai.cli.interactive_runner import list_workflows
