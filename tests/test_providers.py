...
class TestOpenAIProvider:
    ...
    @patch("test_ai.providers.openai_provider.OpenAI")
    ...
    def test_initialize_without_package(self, openai_config):
        ...
        with patch("test_ai.providers.openai_provider.OpenAI", None):
            provider = OpenAIProvider(config=openai_config)
...
class TestAnthropicProvider:
    ...
    @patch("test_ai.providers.anthropic_provider.anthropic")
    ...
    def test_initialize_without_package(self, anthropic_config):
        ...
        with patch("test_ai.providers.anthropic_provider.anthropic", None):
            provider = AnthropicProvider(config=anthropic_config)
            with pytest.raises(ProviderNotConfiguredError):
                provider.initialize()
...
class TestProviderManager:
    ...
    def test_register_explicit_default(self, manager, openai_config, anthropic_config):
        ...
        provider = manager.register(
            "anthropic", config=anthropic_config, set_default=True
        )
        assert manager.get_default() is provider
    ...
    def test_complete_with_fallback(
        ...
        with patch.object(
            openai_provider, "complete", side_effect=ProviderError("API down")
        ):
    ...
    def test_complete_all_providers_fail(
        ...
        with patch.object(openai, "complete", side_effect=ProviderError("Error 1")):
            with patch.object(
                anthropic, "complete", side_effect=ProviderError("Error 2")
            ):
    ...
    def test_complete_rate_limit_fallback(
        ...
        with patch.object(
            openai, "complete", side_effect=RateLimitError("Rate limited")
        ):
            with patch.object(anthropic, "complete", return_value=response):
                result = manager.complete(completion_request)
                assert result.content == "Success"

    def test_generate_convenience_method(
        self, manager, openai_config, completion_response
    ):
...
class TestModuleFunctions:
    ...
    def test_get_provider_default(self):
        ...
        manager = get_manager()
        provider = manager.register(
            "openai", provider_type=ProviderType.OPENAI, api_key="key"
        )
