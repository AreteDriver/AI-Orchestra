...
@pytest.fixture
def mock_settings():
    with patch("test_ai.api_clients.gmail_client.get_settings") as ms:
...
class TestGmailAuthenticate:
    ...
    def test_authenticate_success_existing_token(self, mock_settings):
        ...
        with (
            ...
            patch(
                "test_ai.api_clients.gmail_client.GmailClient.authenticate"
            ) as mock_auth,
        ):
    ...
    def test_list_messages_with_service(self, mock_settings):
        ...
        with patch.object(
            client, "_list_messages_with_retry", return_value=[{"id": "msg1"}]
        ):
    ...
    def test_list_messages_exception(self, mock_settings):
        ...
        with patch.object(
            client, "_list_messages_with_retry", side_effect=Exception("API error")
        ):
    ...
    def test_get_message_exception(self, mock_settings):
        ...
        with patch.object(
            client, "_get_message_with_retry", side_effect=Exception("fail")
        ):
            result = client.get_message("msg1")
