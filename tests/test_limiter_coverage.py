...
class TestTokenBucketLimiter:
    ...
    def test_acquire_exceeds_max_wait(self):
        config = RateLimitConfig(
            requests_per_second=0.1, burst_size=1, max_wait_seconds=0.01
        )
        limiter = TokenBucketLimiter(config)
    ...
    def test_get_stats(self):
        config = RateLimitConfig(
            requests_per_second=10, burst_size=20, name="test-limiter"
        )
        limiter = TokenBucketLimiter(config)
...
class TestSlidingWindowLimiter:
    ...
    def test_get_stats(self):
        limiter = SlidingWindowLimiter(
            requests_per_window=10, window_seconds=60.0, name="test-sw"
        )
