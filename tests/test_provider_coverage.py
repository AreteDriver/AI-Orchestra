...
class TestAnthropicProviderInit:
    def test_init_without_package(self):
        with patch.dict("sys.modules", {"anthropic": None}):
            # Force reimport
            import importlib
            from test_ai.providers import anthropic_provider
...
class TestOpenAIProviderComplete:
    ...
    @patch("test_ai.providers.openai_provider.AsyncOpenAI")
    ...
    def test_complete_rate_limit(self, mock_async_cls, mock_cls):
        from test_ai.providers.openai_provider import OpenAIProvider
        import test_ai.providers.openai_provider as oai_mod
    ...
    def test_complete_no_usage(self, mock_async_cls, mock_cls):
        ...
        mock_response = SimpleNamespace(
            choices=[mock_choice], model="gpt-4o", usage=None, id="x"
        )
        mock_client.chat.completions.create.return_value = mock_response
        mock_cls.return_value = mock_client
    ...
    def test_complete_with_stop_sequences(self, mock_async_cls, mock_cls):
        ...
        mock_client = MagicMock()
        mock_choice = SimpleNamespace(
            message=SimpleNamespace(content="Done"), finish_reason="stop"
        )
        mock_response = SimpleNamespace(
            choices=[mock_choice],
            model="gpt-4o",
