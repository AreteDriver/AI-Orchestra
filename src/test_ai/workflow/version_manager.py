...
class WorkflowVersionManager:
    ...
    def save_version(
        ...
        if existing:
            logger.info(
                f"Content unchanged for {workflow_name}, "
                f"matching version {existing['version']}"
            )
    ...
    def set_active(self, workflow_name: str, version: str) -> bool:
        ...
        if not self._version_exists(workflow_name, version):
            raise ValueError(
                f"Version {version} doesn't exist for workflow {workflow_name}"
            )
    ...
    def rollback(self, workflow_name: str) -> WorkflowVersion | None:
        ...
        for v in versions:
            if v.get_semantic_version() < active_sem:
                ...
                logger.info(
                    f"Rolled back {workflow_name} from {active.version} to {v.version}"
                )
                return v
        ...
        for yaml_file in workflows_dir.glob("*.yaml"):
            try:
                ...
                if self.get_latest_version(workflow_name):
                    logger.debug(
                        f"Workflow {workflow_name} already has versions, skipping"
                    )
                    continue
    ...
    def _set_active_internal(self, workflow_name: str, version: str) -> None:
        ...
        self.backend.execute(
            "UPDATE workflow_versions SET is_active = TRUE "
